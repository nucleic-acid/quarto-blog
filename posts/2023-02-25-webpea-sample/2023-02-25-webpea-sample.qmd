...
Common forms to save plots in R include PNG or SVG. Both have their advantages, be it e.g. versatility or scalability. Since I learned about the [WebP image file format](https://en.wikipedia.org/wiki/WebP) recently and that it is supported by all major web browsers^[[https://caniuse.com/webp](https://caniuse.com/webp)]. The format offers considerable compression improvements over PNG with no apparent visual accuracy loss (in default settings).

![Photo by <a href="https://unsplash.com/@jean_mi_g?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText">Jean-Michel GARCIA</a> on <a href="https://unsplash.com/de/fotos/bwKUJ3Y5JS4?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText">Unsplash</a>](assets/jean-michel-garcia-bwKUJ3Y5JS4-unsplash.jpg)
...
In comes **{webpea}**, an experimantal R package, that I wrote to ease this process. It's just a small package and at this stage a simple wrapper function, that combines `{ggplot2}`'s `ggsave()` with the `{magick}` package. For the basic implementation, please refer to the manual or the [README](https://github.com/nucleic-acid/webpea).
...
In "default quarto rendering" the above plot will be included as PNG with a resolution of 1536 x 1152 pixels and a size of 641,73 KB. 
